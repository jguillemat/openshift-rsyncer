...
function umount_volumes() {
    ...
    log_msg " ------------------------------------------------"
    log_msg " Umount GlusterVol (SOURCE)"
    log_msg " ------------------------------------------------"
    ...
    log_msg " ------------------------------------------------"
    log_msg " Umount NFS Endpoint in remote server"
    log_msg " ------------------------------------------------"
    ...
    then
        log_msg "REMOTE: NFS Endpoint mounted in ${g_remote_replica_dir}."
        execute_remote "umount ${g_remote_replica_dir} --force"
    fi
  
    log_msg "End of Volume umount"
    return "$E_NOERROR";
}
...
for line in $list; do
    ...
    do
        ...
        if [ -n "$p_namespace" ] && [ -n "$p_pvc" ] && [ -n "$p_mount_data" ] && [ -n "$p_pvc_replica" ]; then
            log_msg "Calling umount_volumes method"
            umount_volumes ${p_namespace} ${p_pvc} ${p_mount_data} ${p_pvc_replica}
            if [ $? == 0 ]; then
                log_msg "umount_volumes exists ok"
            else
